...
const ChatInterface = () => {
  ...
  // Inline SelectedProductPreview component
  const SelectedProductPreview = ({ selectedProduct, onClear }: { selectedProduct: Product, onClear: () => void }) => (
  ...
  return (
    <div className="flex flex-col h-full max-w-5xl mx-auto bg-white rounded-2xl shadow-xl border border-gray-200/50 overflow-hidden">
          ...
          <div key={index}>
            ...
            >
              <div className="flex items-start gap-3 max-w-[85%] lg:max-w-[75%]">
                <div
                  className={`px-4 py-3 rounded-2xl shadow-sm hover-lift ${message.role === 'user'
      ...
      {/* Selected Product Preview - appears above input area, below chat messages */}
      {selectedProduct && (
        <SelectedProductPreview
          selectedProduct={selectedProduct}
          onClear={handleClearSelectedProduct}
        />
      )}
      ...
      <div className="border-t border-gray-200/50 bg-white p-4 flex-shrink-0">
        <div className="flex gap-3 items-end">
          ...
          >
            <Send className={`w-5 h-5 transition-transform duration-200 ${isLoading ? 'animate-pulse' : 'group-hover:translate-x-0.5'
              }`} />
          </button>
        </div>
